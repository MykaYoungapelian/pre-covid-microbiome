---
  title: "full control phyloseq"
author: "Youngapelian"
date: "2/26/2022"
output: html_document
---
  
# this contains the script to generate the control mock plots and the control negative template plots. 
...
# ---- load data, subset samples, build phyloseq object ---- ####
...
# ---- filter phyloseq object mock controls ---- ####
rank_names(phyloseq)
get_taxa_unique(phyloseq, "Kingdom")
get_taxa_unique(phyloseq)
phyloseq_filtered_mock <- phyloseq
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_type == "Throat_swab")
#phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_source == "Human")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "Swab_Fernanda")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "Swab_Shu")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "Swab_Paul")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "Swab_Neg")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "Negative_control")
phyloseq_filtered_mock <- subset_samples(phyloseq_filtered_mock, !sample_ID_provider == "NTC")

#save files to produce otuTableControl and taxTableControl
#write.table(otu_table(phyloseq_filtered_mock), file = "phyloseq_filtered_control_otu.txt", sep = "\t") 
#write.table(tax_table(phyloseq_filtered_mock), file = "phyloseq_filtered_control_tax.txt", sep = "\t")

# build phyloseq object to run script directly 
phyl_utr <- phyloseq_filtered_mock
...
plot_bar(Genus, fill = "Genus") + geom_bar(aes(color=Genus, fill=Genus), stat="identity", position="stack")
#ggsave(filename = "plotTaxa.pdf", width = 20, height = 14, units = "cm")
plot_bar(physeq_gen_2, fill = "Genus") + geom_bar(aes(color=Genus, fill=Genus), stat="identity", position="stack") +
  ggtitle("Top 10 Taxa (excluding other)") +
  xlab("Sample") + ylab("Relative Abundance")
#ggsave(filename = "plotTaxaTop20.pdf", width = 16, height = 12, units = "cm")

newtab <- data.table(gg_barplot$data)
newtab$sample_type <- factor(newtab$sample_type)
ggplot(newtab, aes(y = Abundance, x = sample_type, fill = Genus)) + facet_grid(.~ Genus) + geom_bar(stat = 'identity') + theme(legend.position = "right") + theme(axis.text.x = element_text(angle = 90))
#ggsave(filename = "plotTaxaTop10CovidNewTab.pdf", width = 36, height = 14, units = "cm")

# compositional print out 
...
# change name to reflect mock or negative template
...
# plots of mock controls and ungrouped taxa mock control
...
# creating stacked bar graph on abundance and sample_type
...
# ---- filtered phyloseq object negative templates ---- ####
...
# ungrouped negative template plot
{plot.composition.relAbun <- plot_composition(physeq_gen_RA, 
                                              sample.sort = NULL,
                                              otu.sort = names(sort(
                                                taxa_sums(physeq_gen_RA), F)[1:21]),
